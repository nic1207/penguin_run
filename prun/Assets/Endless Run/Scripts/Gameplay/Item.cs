...
public class Item : MonoBehaviour {
	...
	public int scoreAdd; //add money if item = coin
	...
	//Set item effect
	public void ItemGet(){
		if(GameAttribute.Instance.deleyDetect == false){
			if (typeItem == TypeItem.Coin || typeItem == TypeItem.Fish) {
				HitCoin ();
				//Play sfx when get coin
				if (typeItem == TypeItem.Fish)
					SoundManager.Instance.PlaySE ("eat_fish", false);
				else
					SoundManager.Instance.PlaySE ("eat_flag", false);
			} else if (typeItem == TypeItem.Obstacle) {
				HitObstacle ();
				initEffect (effectHit);
				//Play sfx when get hit
				SoundManager.Instance.PlaySE ("hit", false);
				...
				if (Controller.Instance.timeSprint <= 0)
					Controller.Instance.Sprint (itemEffectValue, duration);
				else
					Controller.Instance.timeSprint = duration;
			...
			*/
				...
				HideObj ();
				initEffect (effectHit);
			} else if (typeItem == TypeItem.Crystal) {
				HitObstacle ();
				initEffect (effectHit);
				//Play sfx when get hit
				SoundManager.Instance.PlaySE ("hit", false);
				Controller.Instance.SpeedDown(5, 3);
			} else if (typeItem == TypeItem.Clock) {
				SoundManager.Instance.PlaySE ("eat_flag", false);
				HideObj ();
				initEffect (effectHit);
				if(timeAdd>0)
					GameAttribute.Instance.time += timeAdd;
				//Play sfx when get hit
				//SoundManager.Instance.PlaySE ("hit", false);
				//Controller.Instance.SpeedDown(5, 3);
			}
...
*/
	...
	private void HitCoin(){
		...
		GameAttribute.Instance.fish += scoreAdd;
		//}else{
		//	GameAttribute.gameAttribute.fish += (scoreAdd)*GameAttribute.gameAttribute.multiplyValue;
		//}
//		InfiniteRunner.Game.CoinGUICollection.instance.CoinCollected (scoreAdd, false);
		initEffect(effectHit);
		HideObj();
	}
	...
	private void HitObstacle(){
		//if(GameAttribute.gameAttribute.ageless == false){
			...
			//}else{
		HideObj();
		GameAttribute.Instance.ActiveShakeCamera();
			//}
		//}
	}
	...
	private void initEffect(GameObject prefab){
		...
		if (!prefab)
			return;
		GameObject go = (GameObject) Instantiate(prefab, Controller.Instance.transform.position, Quaternion.identity);
		go.transform.parent = Controller.Instance.transform;
		go.transform.localPosition = new Vector3(go.transform.localPosition.x, go.transform.localPosition.y+2.0f, go.transform.localPosition.z);	
	}
	...
	public IEnumerator UseAbsorb(GameObject targetObj){
		...
		useAbsorb = true;
		while(isLoop){
			this.transform.position = Vector3.Lerp(this.transform.position, targetObj.transform.position, GameAttribute.Instance.speed*2f * Time.smoothDeltaTime);
