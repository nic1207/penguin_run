	...
	public class IKSolverFullBodyBipedInspector : IKSolverInspector {
		...
		public static void AddInspector(SerializedProperty prop, bool editHierarchy, bool editWeights, SerializedContent[] content) {
			...
			if (content[4].prop.isExpanded) {
				AddPulls(content[4].prop.FindPropertyRelative("children"));
				EditorGUILayout.Space();
			}
		...
		private static void AddLimbHelper(FBIKChain chain, float size, Transform root = null) {
			Vector3 cross = Vector3.Cross(chain.nodes[1].transform.position - chain.nodes[0].transform.position, chain.nodes[2].transform.position - chain.nodes[0].transform.position);

			Vector3 bendDirection = -Vector3.Cross(cross, chain.nodes[2].transform.position - chain.nodes[0].transform.position);
		...
		private static void AddPulls(SerializedProperty baseChildren) {
			if (baseChildren.arraySize < 4) return;
			
			AddPull(baseChildren.GetArrayElementAtIndex(0), new GUIContent("Left Arm", string.Empty));
			AddPull(baseChildren.GetArrayElementAtIndex(1), new GUIContent("Right Arm", string.Empty));
			AddPull(baseChildren.GetArrayElementAtIndex(2), new GUIContent("Left Leg", string.Empty));
			AddPull(baseChildren.GetArrayElementAtIndex(3), new GUIContent("Right Leg", string.Empty));		
		}
